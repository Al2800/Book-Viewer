...
The normal launch now defaults to Reading / Capture / Studio; early default-off language describes the historical rollout. Coherent-UI refinements are tracked in `book-quote-coherent-ui-5sy4`, not a new shell version. Reading and Settings now share List as the unset browse preference, preserving explicit Shelves/Grid/List selections. Settings includes every browse mode and explicit dismissal when presented from Reading. Follow-up implementation now shares capture orchestration across tab/Reading/Book Detail, returns saved passages to the same-book camera, exposes single-page source inspection and counted review save, and hosts one shared Studio editor without a preview lobby. Review now supports explicit candidate selection, preserves active-review edits/selection during polling, and recovers partial saves by submitted identity. Forced persistence-failure tests cover failed inserts and duplicate-free retry. Review also presents explicit checking guidance, distinguishing missing confidence and manual entry without claiming verification or changing selection thresholds. Live search results can now be opened without keyboard submission. Continue Reading now uses a compact active-book row and separate Capture action; organization filters are explicitly scoped beside Books. Shelves include every reading status and support editing/deletion actions, with item-driven book edit presentation preventing blank sheets. Recent Passages now links to an unfiltered All Passages list with canonical detail navigation; Revisit only draws from older material outside the latest three. Books controls adapt to large text, and shelves use readable grouped rows at larger text sizes without changing the saved preference. Studio previews and exports now share a fixed canonical layout with exact pixel-comparison coverage. Measured fit/crop checks prevent unsafe image exports while preserving Markdown; deleted selection requires explicit replacement, and injected Photos/clipboard failures verify recovery. Review corrections confirmed in the editor, manual entries and selection now checkpoint durably without a SwiftData schema change; each saved quote and candidate removal commit atomically. Process-relaunch UI tests verify recovery, preserved source pixels and no resurrection of saved candidates. These are implemented portions, not completed beads: the remaining accessibility/performance/retirement evidence is still open. Mixed failed pages now remain inspectable after successful saves, with explicit confirmed manual resolution and real on-device retry covered by phone/iPad simulator tests; saved candidates are not reimported. Review now labels extra-checking counts without implying verification and uses cancellation-aware, viewer-cached source loading; iPad simulator count/save/source checks pass. Supporting sheets/actions now use Passage wording, and the shared UI harness defaults to the current shell with explicit legacy smoke coverage. The resolved-path audit confirms all 124 unit-test Swift files are compiled. A discovered fixture-registration crash was fixed and passed ten repeated launches plus final regression. Foundation `.1` is now accepted with measured light/dark contrast, current-shell label/trait/hit-region audits, XXXL journeys and an executed real-system Reduce Motion check; the four feature beads and release/device gates remain separate. See [current UI contract](../UI_COMPONENTS.md#current-coherent-ui-contract-2026-09-07).
