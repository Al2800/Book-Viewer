...
Normal launch and the shared UI-test harness now use the three-tab shell by default. Legacy presentation remains an explicit opt-in release-gated fallback, with a dedicated Library/Settings access smoke test, not a second long-term architecture. UI fixtures establish deterministic identities before managed relationships and finish seeding before test tab queries mount; this avoids registration races without changing production startup or persistence schema. `LibraryViewMode` owns the shared preference key, default and presentation names; both Reading and Settings bind that same enum through `@AppStorage`. Raw persisted values remain unchanged. The Settings sheet passes an optional close action into the existing `SettingsTab`; legacy tab presentation does not acquire a dismiss button. Shared controls are changed in place rather than wrapped in a new UI version. Unfilled actions use the adaptive `actionForeground` role; `brand` remains the filled-control/switch colour, and `destructiveFill` separates white-button labels from the dark-mode error status accent. Foundation acceptance is recorded; physical file retirement remains approval-gated. See [current UI contract](../UI_COMPONENTS.md#current-coherent-ui-contract-2026-09-07) and bead `book-quote-coherent-ui-5sy4.1` for the remaining retirement gate.
