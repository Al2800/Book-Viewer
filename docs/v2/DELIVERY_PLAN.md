...
The default-off shell below is historical. Normal launch currently uses Reading / Capture / Studio. The five refinement workstreams are tracked by `book-quote-coherent-ui-5sy4`; foundation `.1` has passed its shared-contract acceptance; the four feature beads remain unfinished. Its first slice replaces conflicting browse defaults/Settings options, stale Explore/search terminology, fixed-size shared typography and undersized shared controls. Follow-up code now includes canonical capture entry/same-book save return, single-page source inspection/counted review save, and one shared Studio workspace with explicit passage selection and export outcomes. Reading hierarchy, durable review, mixed-page recovery and source loading have since advanced with evidence in the current UI contract. Supporting controls now use Passage wording, and tests default to the current shell rather than silently exercising legacy navigation. All 124 unit-test Swift files are registered by resolved project path. Broader acceptance gates remain open; code landing is not equivalent to bead completion. The legacy shell's retirement requires route/test parity evidence before removal. Executed checks and artifact paths are recorded on the foundation bead; a simulator check must not be represented as physical-camera or live extraction validation.
