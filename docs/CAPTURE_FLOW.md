...
### Build 61 device-feedback corrections — 2026-09-08

Page capture now skips automatic Vision rectangle cropping: a detected rectangle can be a text block, removing page edges/marginalia and making the result look zoomed. The quote-page processor retains the original UIImage, orientation, scale and pixels for quality analysis and subsequent source handling, including analysis failure; cover-specific preparation is unchanged. This addresses a concrete automatic-crop path, not a claim that physical camera framing has been verified. Existing already-cropped captures cannot be restored by this change.

Review Save now uses a native bordered-prominent toolbar button with the existing brand fill/white label instead of nesting a custom filled rectangle inside iOS 26 glass. Selection disabling, counted labels and save ownership are unchanged. Light screenshot evidence in `device-feedback-20260908-47-attachments` confirms the nested square is gone; three selection/capture/accessibility journeys pass. `device-feedback-final-20260908-48.xcresult` passes 787 units (one optional fixture skipped) and the dark review audit. Fixes are source changes after Build 61, not yet a new TestFlight upload. Tracking: `book-quote-vv8a`, `book-quote-ef4h`; extraction connectivity diagnosis remains separate under `book-quote-cbjb`.

### Durable review drafts

Review now checkpoints committed editor corrections, manual additions, selection, candidate identity and loaded-page bookkeeping in a versioned envelope inside existing `PageCapture.extractedQuotesData`. Original extraction bytes remain intact; legacy arrays remain readable and there is no SwiftData schema change. Cancel offers **Keep Draft** or explicit discard. Saved Drafts distinguishes capture drafts from review drafts; completed extraction can still have a resumable review. The review owns its navigation stack rather than inheriting the camera's hidden toolbar.

Each confirmed quote save atomically commits removal of that candidate from the checkpoint in the same context save. Failed/excluded candidates and their source remain available; a failed write restores prior checkpoint bytes and failed quote inserts cannot leak into a later save. Repeated/stale candidates are refused. Interrupted processing resets only unfinished pages, preserving completed extraction and ignoring cancelled late results. Recovery errors are surfaced before enabling review mutations; retry does not overwrite unreadable checkpoint data.

Evidence: `review-durability-regression-20260907-23.xcresult` has 777 unit tests (one optional fixture skipped) and five passing UI journeys, including real process relaunch with and without explicit Keep Draft, retained source pixels, manual entries/selection, subsequent save/resume, ordinary batch draft and repeated same-book capture. Relaunch tests opt into a UUID-namespaced disk store via `--uitesting --ui-test-store-id <UUID>`; ordinary UI tests remain in memory, and production storage is not opened. `review-durability-final-20260907-24.xcresult` has 778 unit tests (one optional fixture skipped) and five passing UI tests, including adaptive portrait/landscape/source inspection and duplicate/selection regression. Initial relaunch testing exposed the hidden toolbar; it was fixed rather than bypassed. This does not cover unsaved keystrokes inside an unconfirmed editor, physical interruption or disk-full hardware conditions. Subsequent mixed-page recovery tests are recorded in the current UI contract: successful candidates save without losing failed sources; explicit manual completion and real on-device retry do not resurrect saved candidates. Manual completion is separately confirmed and checkpointed, with page/session status restored on write failure. Failed pages retain scoped source/manual actions even when no successful candidates remain.
