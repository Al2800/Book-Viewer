...
- `actionForeground` is for unfilled actions/links/selection (brand ink in light appearance, primary ink in dark); `brand` remains a filled-control/switch swatch. `destructiveFill` keeps the existing light error swatch behind white labels rather than using the lighter dark-mode status accent. Measured functional text and enabled-action label pairs meet 4.5:1 in both appearances across the three neutral surfaces.
...
`review-durability-regression-20260907-23.xcresult` passes 777 unit tests (one optional fixture skipped) and five UI journeys including two actual process-relaunch scenarios, retained source pixels, manual entries/selection, save/resume, ordinary batch draft and repeated capture. Tests use an isolated UUID-namespaced disk store, not a same-process in-memory recreation. `review-durability-final-20260907-24.xcresult` passes 778 unit tests (one optional fixture skipped) and five adaptive/selection/duplicate UI journeys. The initial relaunch failure exposed a real hidden-navigation-toolbar bug: resumed review now owns its stack. Migrated a stale source-accessibility test that only asserted control absence into actual source opening/image checks. At that checkpoint, remaining gates included mixed extraction failures/manual-resolution E2E, checking counts, source loading and broader device evidence; the count/loading/iPad follow-up below supersedes those portions. Unconfirmed text inside an open editor is not checkpointed per keystroke.

**Review counts/source loading/iPad follow-up:** a labelled extra-checking count covers all displayed candidates, including excluded results. It uses the existing guidance bands, treats unavailable scores neutrally, excludes manual entries from model uncertainty and explicitly reminds readers to compare every passage; edits never imply verification. Full source inspection now holds one image for the viewer lifetime rather than synchronously reading it during every pan/zoom redraw. Extraction and inspection share a cancellation-aware background file loader; newer requests and cancellation prevent stale pixels. Loading and unavailable states are distinct. Whole-page inspection remains the honest fallback; no bounds overlay is invented.

`review-source-regression-20260907-27.xcresult`: 783 unit tests, one optional fixture skipped, zero failures, plus a passing process-relaunch/source-pixel journey. New unit tests cover count boundaries/edit/selection behaviour, cache reuse, unavailable sources, cancellation and out-of-order completion. `review-source-counts-20260907-25.xcresult` passes three adaptive phone review UI tests; `review-ipad-20260907-26.xcresult` passes actual iPad Pro 13-inch simulator checking-count, save and source-open/return assertions. This is simulator evidence, not physical-device or VoiceOver sign-off. Mixed-failure/manual-resolution and unavailable-source UI evidence is added in the follow-up below; broader physical/VoiceOver and retirement gates remain.

**Mixed-page recovery follow-up:** failed pages stay in the same page-group review even after all successful candidates have saved. They retain their source action, actual error, scoped manual entry and retries. Manual entry uses item-driven presentation to keep the correct page. A separate confirmed **Finish manual review for PAGE N** action records the user's decision; adding one passage never silently declares a page complete. Resolution and loaded-page bookkeeping commit with the checkpoint, preserving source/raw bytes; injected write failure restores page/session status and checkpoint. Resumed review cannot reimport already saved source candidates. This is manual resolution, not verified AI output.

`review-mixed-regression-20260907-30.xcresult` passes 784 unit tests (one optional fixture skipped) and seven UI journeys, including save-successes/retain-failure/source/manual-resolution, real on-device retry of the failed fixture page with no resurrected candidates, unavailable-source display with retained review, durable relaunch and adaptive layouts. `review-mixed-ipad-20260907-31.xcresult` passes three iPad Pro simulator journeys covering count/source, mixed manual recovery and unavailable source. Fixtures are opt-in UI-test scenarios; the missing-source fixture clears only its reference and does not delete a file. Production extraction/consent policy is unchanged. These are not physical camera, live remote-provider or VoiceOver walkthrough results.

**Supporting sheets and current-shell parity follow-up:** live editor/manual, collection, export, duplicate, onboarding, Settings and card action labels now use Passage without model/export-schema renames. The editor scrolls to retain fields above a keyboard; the manual sheet appears immediately and disables Add for empty/whitespace input instead of using decorative entrance/shake state. Legal/brand terminology, fixture prose, symbols and stored identifiers are not mechanically renamed.

The shared UI harness now defaults to Reading/Capture/Studio. Retained legacy access is explicitly requested and smoke-tested, not an accidental default. Migrated stale Library/title/menu expectations; large-text Settings tests scroll to offscreen controls rather than assuming every control fits above the fold. Resolved-path PBX audit found all 124 unit-test Swift files compiled, with no omissions or stray references. Current-shell Settings/accessibility/search coverage passed in `foundation-current-shell-20260907-34.xcresult` (its two stale onboarding assertions were corrected later); collection/Studio checks passed in run 35.

Run 35 also exposed a genuine SwiftData duplicate-registration trap during fixture startup. `book-quote-sxt2` fixes ID assignment before managed relationship attachment and waits for fixture commit before mounting test tab queries. Original crash/stderr diagnostics remain under `foundation-parity-20260907-35-*`. Ten fresh-process startup/All Passages repeats pass in `fixture-startup-repeat-20260907-36.xcresult`. Final `foundation-final-20260907-37.xcresult` passes 784 unit tests (one optional fixture skipped) and 20 UI tests, including full onboarding, current/explicit-legacy shells and manual recovery. The scoped crash bead is closed; this does not close the five feature beads. The contrast/motion/accessibility and retirement-disposition acceptance follow-up below supersedes that remaining-work checkpoint.

**Foundation acceptance (2026-09-08):** functional contrast now has numerical tests using resolved asset colours, not a declaration-only token check. The initial 4.43:1 secondary-text failure was corrected without relaxing 4.5:1. Unfilled actions no longer reuse a dark fill colour in dark appearance; filled switches retain brand tint and destructive labels retain a sufficiently dark fill. View page now has an explicit content-shaped 44pt hit region. Marking selection exposes a selected trait, and ornamental Studio quotation/pencil/cover/ribbon elements are hidden from assistive technology while authored text remains readable. Exact Studio bitmap comparisons still pass.

`foundation-acceptance-20260908-45.xcresult`: 786 unit tests (one optional fixture skipped), 11 UI tests (one opt-in motion check skipped under the restored standard setting), zero failures. The real system Reduce Motion check executed and passed separately in `foundation-motion-20260908-41.xcresult`, checking the actual SwiftUI environment and navigation, not a fake preference flag. Simulator motion was restored to its original false setting. `foundation-dark-final-20260908-40.xcresult` passes five dark-mode UI audits; Studio passes light/dark audits in runs 43/44. Final dark review/Studio screenshots and audits pass in `foundation-dark-evidence-20260908-46.xcresult`; appearance was restored to light. Audits check hit regions, meaningful labels and traits; numerical contrast and dedicated XXXL journeys are separate evidence. These establish accessibility semantics, not a physical spoken-VoiceOver walkthrough.

Foundation `.1` is accepted for its shared UI contract. Retirement disposition: no new shell/version or persistence aliases; default tests/callers use the current shell; the old shell is an explicitly tested fallback under the existing release/retirement gate. File removals remain deferred to the approval-controlled cleanup bead as requested. This does not certify deletion, finish the four feature beads, or authorize a claim of physical-camera/Photos/TestFlight verification.
